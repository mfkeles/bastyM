...
for i=1:numel(folder_list)
    ...
    for j = 1:size(prob_dat,1)
        ...
        indv_event = temp_arr(idx(1):idx(2),[1,2,12,16]); %normally 1,2,8,10)
...
for i=1:size(fly_table,2)
    ...
    maxes = max(ceil(cellfun(@(x) max(x(:,3)),fly_table{idxes,i})));
    mins = min(floor(cellfun(@(x) min(x(:,3)),fly_table{idxes,i})));
    ...
    for j=1:numel(idxes)
        ...
        plot(pl_arr(:,3),'LineWidth',1,'Color',[0.8500 0.3250 0.0980]);
        box off
        make_it_black;
        set(gca,'TickDir','out')
        ...
        %calc table name
        dur = AuxFunc.frame_to_hours(round(mean([prob_locs(j,1) prob_locs(j,2)])),30);
        ztTime(j) =round(rem(10 + hours(dur),24),2);
        trueZT(j) = round(10+hours(dur),2);
        pump_strength(j) = round(length(pl_arr(:,3))/90);
